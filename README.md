...
- 2026-07-22 · "Build My Kit" CTAs skip the /start path chooser: the hero "Start the Quiz" already went straight to `/quiz`, but every "Build My Kit" CTA (SiteNav desktop + mobile, SiteFooter, homepage bottom CTA, 404) still routed to `/start` — so the primary CTA dropped home-gym users on the home-vs-pro chooser. Pointed all five at `/quiz` to match the hero and remove the extra click for the main (home-gym) funnel; the For Gyms/pro path stays reachable via the always-visible "For Gyms" nav link (→ /gym). `/start` kept as a valid page (still the FloorPlanner back-fallback for unknown origins) · (this commit)
