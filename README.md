...
| `src/lib/deals.ts` | Deals engine — `MIN_DEAL_PCT` (5% floor), `settleSale` (applied at the catalog boundary), `productDeal`/`findDeals` (curated `salePrice` only), `saleExpired`, `rankWithDeals` (bounded deal boost), honest countdowns; AI pitch layer from `src/data/deal-pitches.json` |
...
| `src/lib/auto-layout.ts` | Auto-layout engine: client-side wall detection from the uploaded schematic (Otsu threshold → dilate → flood-fill interior; photos fall back to borders-only) + formation packer (cardio rows w/ fall zones, racks backed to walls, dumbbells+facing benches, machines w/ aisles). each piece padded by its own halo (not a flat worst-case ring) so results never render red and small gear still fits; gated by `npm run audit:layout` |
...
Deterministic wall (same as the kit builder): **the LLM never sources a price or expiry** — deals derive only from curated `salePrice`/`saleEndsAt` in server.js; Groq writes copy over computed numbers, weekly, as a cached artifact. `lib/catalog.ts` strips expired sale fields at fetch time, so every surface (gear, category, compare, kit, search) drops a dead sale automatically within the hourly ISR window; the client deal strip re-checks live via `productDeal()`. A discount under **5%** is not a deal: `settleSale` collapses it into the plain price (the customer still pays the lower number — deleting the field would requote list), and only 5%+ gets a badge, the top-of-category **On sale right now** strip, and a bounded rank boost (cap 5 pts, since category scores cluster in a ~10-point band). Audit the catalog with `npm run check:deals`; the rules themselves are gated by `npm run audit:deals`. The backend re-checks every product's discount **daily** (`price-check` workflow) and opens a PR. Note: countdown labels only render when a sale has a curated `saleEndsAt` — as of 2026-07-09 none do, so add dates in server.js to activate them (never invent one).
...
- 2026-08-10 · a retired category now 404s even when stale product data lingers (the page only checked for an empty ranking, and Vercel's persisted fetch cache kept re-rendering "Best Hoodies" after the category was gone); category list revalidates at 5 min instead of 60 · this commit
...
- 2026-08-03 · Auto-arrange padded every piece with a flat 17" ring (worst-case rack halo), so small gear claimed a rack's floor and a 12×14 ft room took only 6/12 of a kit — dropping the mat, roller, rope, barbell and the accessory storage shelf, i.e. the empty room the 3D models were meant to fix. `canPlace` now records each occupied cell's own halo and requires only the two halos summed, matching the rule the planner paints red: 12×14 → 9/12, 14×18 → 12/12, zero overlaps at any size. Gated by new `npm run audit:layout` (placement counts + halo overlaps + out-of-room across four room sizes) since `autoPlace` never throws, it drops pieces silently · 7802d27
- 2026-07-23 · All quiz-entry CTAs land on the `/start` chooser (home-gym vs pro-gym): reverted "Build My Kit CTAs skip /start" (commit 845f030) — nav (desktop + mobile), footer, homepage bottom CTA, and 404 back to `href="/start"` — AND routed the homepage hero "Start the Quiz" button to `/start` too, so every quiz entry point now goes through the chooser first (Roe's call). `/quiz` is still reachable directly and via the chooser's home-gym path · (this commit)
- 2026-07-22 · Final QA sweep fixes: (1) search now matches multi-word queries — `SearchModal` `rank()` split the query into words and requires each to land across name+brand+category, so a natural "rogue rack" finds Rogue racks (was: whole-query substring only → "No matches"); (2) stale copy sweep — catalog is 31 categories, not 29: fixed all four user-facing "29 categories" claims (homepage how-it-works card + hero stat line, QuizFlow building-screen text, sponsors stat grid) to drift-resistant "30+ categories" matching the sibling "250+ products". Full page-by-page walkthrough (quiz→kit→swap, /gym→backend plan→/planner handoff→auto-arrange 42/42→3D, compare/extras/gear/guides/category/support) verified green; build clean · (this commit)
- 2026-07-16 · Floor dashboard = a button, not a scroll-down: removed the embedded `<FloorPlanner>` from both KitResult (home gym) and GymPlanner (For Gyms) `#floor-dashboard` sections; the existing entry buttons ("Visualize it in your space" / "Floor plan") now `<Link>` straight to the full-screen `/planner` (kit/plan already synced live via the sessionStorage handoff), which killed the duplicate "Full-screen planner" links (the unnecessary buttons). Gym room auto-sizing preserved by carrying `{w,d}` through `saveFloorItems(items, room)` → new `loadFloorRoom()` → FloorPlanner uses it as the initial room size when no saved layout dims exist (verified: /planner opened at the gym's 66×45, palette seeded, "Back to your plan"→/gym) · (this commit)
- 2026-07-16 · Cinematic 3D inspect: selecting a piece now runs a movie-style reveal — time-eased 1.3s camera dolly (ease-in-out replaces the old lerp), the room fades to a dark spotlight mood (bg/fog/floor/lights lerp + non-selected pieces dim toward the dark), a warm SpotLight (decay 0 — inch units) pools on the hero so its real materials read, and the piece carries a brand-orange emissive tint (2D selected halo now orange too, matching the home cards); the product card is anchored in-scene beside the piece (screen-space projection of its bbox, tracked every frame, orange-glow floating variant) instead of pinned top-right; link audit re-confirmed site-wide (search→/gear, category/gear/guides/kit/compare/gym/planner all link) · (this commit)
- 2026-07-15 · Life-accurate 3D equipment models — all 21 builders rebuilt procedurally (no external model files: licensing + payload + nothing matches the real catalog): new detail vocabulary in equipment-3d (tube-between-points, weight plates w/ hubs, loaded barbells w/ sleeves+collars, selectorized stacks w/ guide rods+pin, pulleys+cables, caged fan wheels, chrome/rubber materials); racks get J-cups/safeties/storage horns/hole rows, machines get real stacks+handles (Tonal articulated arms, 45° leg-press sled, iso-row plate arms), cardio gets fan cages/frame tubes/consoles (C2 rower w/ monorail+chain, echo-bike, connected spin-bike frame), A-frame dumbbell rack w/ hex bells, FID bench w/ wheels; verified wide + close-up captures of every family, fixed spin-bike/GHD/air-bike readability on review · (this commit)
- 2026-07-15 · Click-to-inspect product cards on the planner (2D + 3D): new `EquipCard` (home-FlowCard dark-glass language — grid texture, family-hue glow, image/brand/price/sale/GymGear-Score, "View product" buy link via cached catalog route + per-category cache) overlays whichever view is active; 2D tiles get family-hue hover glow + selected halo, click-vs-drag split at 6px; 3D pieces glow emissive on hover (cursor→pointer) and stronger when selected, click glides the camera into a close-up (lerp target+pos, cancelled by manual orbit), empty space deselects, selection syncs both ways without scene rebuilds; with this every product surface on the site now links out (audit: compare/gear/kit/modal/gym already used buyUrl) · (this commit)
- 2026-07-15 · Planner distilled to functional-only (impeccable distill): removed the "Layout rules that matter" advice sidebar (map now full-width single column), the uppercase eyebrow kicker, the long intro paragraphs (→ one line), the "measure before you drill" disclaimer (density warning stays, conditional), and shortened palette/zone hints; toolbar reordered by importance — Upload → room size → Auto-arrange (primary) → 3D → quiet toggles; page metadata updated · (this commit)
- 2026-07-15 · Manual "off-limits" areas on the planner: new draw tool (`Mark off-limits` toggle) lets the user drag rectangles over the map to box off doors, walls, side rooms or any area to keep clear — one tool covers all three the user asked for; zones (`Zone` type in floor-plan, inches) persist with the layout, render as orange-hatched rects with a per-zone delete + `Clear areas`, fold into the grid via `applyZones` so Auto-arrange AND the 3D walls avoid them, and a piece dragged into a zone flags red (reuses the collision set); pointer-capture guarded. Verified end-to-end: draw→persist→render→9/9 auto-placed with 0 overlaps→manual overlap flags red→delete clears · (this commit)
- 2026-07-15 · Shared branding across 2D↔3D + full-room detection: one `FAMILY_COLORS` source of truth in equipment-icon (racks orange, machines violet, cardio sky, benches rose, dumbbells green) now drives the 2D map tiles + palette icons AND the 3D models, so a category reads the same hue in both views; 3D view gains a GymGear wordmark overlay; auto-layout `mainRoom` room detection rewritten from a capped regrow (which truncated L-shapes/wrap-around corridors) to a nearest-core watershed — the whole training floor comes back in full while side rooms (office/bathroom/store) stay off-limits and doorways split at the midline (verified: synthetic warehouse+office+bathroom → block excluded, left arm + below + right wrap-around strip all usable) · (this commit)
- 2026-07-15 · 3D view showroom restyle: grid removed, light studio scene (bright room floor as stage, darker surround, ACES tone mapping, brighter lights), glass walls lightened (opacity .28, no depthWrite so equipment never dims behind them), equipment rebuilt fuller — graphite steel frames pop on light floor, thicker posts/rails, real bumper-plate pairs on all racked bars, chunkier benches/dumbbells/pads · (this commit)
...
- 2026-07-14 · Site nav on the trap pages: /gym, /planner and /start rendered their tool component with no `SiteNav`, so landing there left no way back to the rest of the site — wrapped all three with `<SiteNav/>…<SiteFooter/>` like every other page (pattern: /compare). (Note: the user's word "dashboard" throughout this work meant the site nav header, not the floor-plan board.) · (this commit)
  - **Correction (2026-07-22):** the parenthetical here that "/quiz stays nav-free by design" was inaccurate — `QuizFlow` has rendered `<SiteNav/>` on every phase since Phase 2 (commit 8fd456e). The quiz is intentionally kept with the site nav like every other page; there is no nav-free-funnel behaviour to preserve.
